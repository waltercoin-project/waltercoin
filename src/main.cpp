...
#if defined(NDEBUG)
# error "Waltercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb44e962bc938fd122db65c7ea00dd28f7b4db57d7fd3a511a9cb957fac033128");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Waltercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Waltercoin Signed Message:\n";
...
{
    // Waltercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Waltercoin: 420k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Waltercoin: 1 days
static const int64 nTargetSpacing = 3 * 60; // Waltercoin: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Waltercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Waltercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfc5b633d737d0d86c5bf09510cf7e3b292d840e009742693becb95602c9a878d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "7 Republicans Break With Former President in Vote on 2nd Impeachment";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045c6b09564e34895bab82857f178c082ce3062d0e410d60dacc5129ff4a8442fd9a4b9016fefc19229f3a7e95bee43f31f1b8ae7ea3cc025b93bceebe56e818f0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1613302942;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 388476431;

        if (fTestNet)
        {
            block.nTime    = 1613302908;
            block.nNonce   = 2087093286;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcf4185c366a151f064f63b541b30bbf4d95046fd893cc36f73bdfaeff88bae2d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x45, 0x52, 0x43, 0x4f }; // Waltercoin: not increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WaltercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WaltercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WaltercoinMiner(CWallet *pwallet)
{
    printf("WaltercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("waltercoin-miner");
    ...
    try { loop {
        ...
        printf("Running WaltercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WaltercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WaltercoinMiner, pwallet));
}
